var jsPsychFingerTapping = (function (jspsych) {
  ...
  class jsPsychFingerTappingPlugin {
    ...
    trial(display_element, trial) {
      ...
      var fingerNamesTranslated = ["left", "right"]
      ...
      d3.select("#targetStr").text('For this trial, alternate between the keys ' + trial.key_targets[0] + ' and ' + trial.key_targets[1])
      ...
      d3.select("#instructionStr").text("with your " + fingerNamesTranslated[trial.finger] + " finger only")
      ...
      d3.select("#svgview").transition().duration(100).attr("opacity", "1").on("end", function () {
        ...
        d3.select("#gaugeText").text("Press the space bar to start.")

var jsPsychFingerTappingDemo = (function (jspsych) {
  ...
  class jsPsychFingerTappingDemoPlugin {
    ...
    trial(display_element, trial) {
      ...
      d3.select("#targetStr").text('Demo trial (just have look)')
      ...
      d3.select("#instructionStr").text("If asked to press " + trial.key_targets[0] + " and " + trial.key_targets[1] + " with your " + fingerTranslatedNames[trial.finger] + " finger")
      ...
      function startTrial() {
        ...
        d3.select("#instructionStr").text("You'll have to keep alternating between buttons until the time bar has elapsed.")
        ...
        d3.select("#timeGaugeBar").transition().ease(d3.easeLinear).duration(trial.trial_duration).attr('width', 0).on('end', function () {
          ...
          setTimeout(function () {
            ...
            d3.select("#instructionStr").text('Press the space bar to continue.')
